 ...
 */
function bloghash_nav_menu_item_args( $args, $item, $depth ) {

	if ( bloghash_option( 'main_nav_sub_indicators' ) ) {
		$dropdown_indicator = bloghash()->icons->get_svg( 'chevron-down' );
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			if ( false === strpos( $args->link_after, $dropdown_indicator ) ) {
				$args->link_after .= $dropdown_indicator;
				$args->after       = '<button type="button" class="bloghash-mobile-toggen">' . $dropdown_indicator . '</button>';
			}
		} else {
		...
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace(
					$args->link_after,
					'</span><span class="' . esc_attr( $value ) . '" title="' . esc_attr( ucfirst( $value ) ) . '">' . bloghash()->icons->get_svg( $value, array( 'aria-hidden' => 'true' ) ) . bloghash()->icons->get_svg(
						$value,
						array(
							'class'       => 'bottom-icon',
							'aria-hidden' => 'true',
...
if ( ! class_exists( 'Bloghash_Icons' ) ) :
	...
	class Bloghash_Icons {
		 ...
		 */
		public function get() {
			return apply_filters( 'bloghash_icons_svg', self::$icons );
		}
		...
		public function get_meta_icon( $slug = '', $icon = '', $post_id = '' ) {
			...
			if ( is_single( $post_id ) ) {
				if ( bloghash_option( 'single_entry_meta_icons' ) ) {
					$return = $icon;
				}
			} elseif ( ! is_single() ) {
				if ( bloghash_option( 'entry_meta_icons' ) ) {
					$return = $icon;
				}
			}

			return apply_filters( 'bloghash_' . $slug . '_meta_icon', $return, $post_id );
		}
