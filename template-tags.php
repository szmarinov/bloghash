...
if ( ! function_exists( 'bloghash_get_schema_markup' ) ) :
	/**
	 ...
	 */
	function bloghash_get_schema_markup( $location = '' ) {

		// Check if schema is enabled.
		if ( ! bloghash_is_schema_enabled() ) {
			return;
		}
		...
		$schema = ' ' . trim( apply_filters( 'bloghash_schema_markup', $schema, $location ) );

		return $schema;
	}
endif;

if ( ! function_exists( 'bloghash_schema_markup' ) ) :
	/**
	 ...
	 * @param string $location Location for schema parameters.
	 */
	function bloghash_schema_markup( $location ) {
		echo bloghash_get_schema_markup( $location ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
endif;

if ( ! function_exists( 'bloghash_logo' ) ) :
	/**
	 ...
	 */
	function bloghash_logo( $echo = true ) {

		$display_site_description = bloghash_option( 'display_tagline' );
		...
		if ( bloghash_has_logo() ) {

			$default_logo = bloghash_option( 'custom_logo', '' );
...
if ( ! function_exists( 'bloghash_hamburger' ) ) :
	...
	function bloghash_hamburger( $button_title, $menu_id ) {
		...
		?>
		<button class="<?php echo esc_attr( $classes ); ?>" aria-label="<?php esc_attr_e( 'Menu', 'bloghash' ); ?>" aria-controls="<?php echo esc_attr( $menu_id ); ?>" type="button">

			<?php if ( $button_title || is_customize_preview() ) { ?>
				<span class="hamburger-label uppercase-text"><?php echo wp_kses( $button_title, bloghash_get_allowed_html_tags( 'button' ) ); ?></span>
			<?php } ?>
...
if ( ! function_exists( 'bloghash_pagination' ) ) :
	/**
	 * Output the pagination navigation.
	 *
	 * @since 1.0.0
	 */
	function bloghash_pagination() {
		...
		<?php
			...
			the_posts_pagination(
				array(
					'mid_size'  => 2,
					'prev_text' => sprintf( '<span class="screen-reader-text">%s</span>', esc_html__( 'Previous page', 'bloghash' ) ) . bloghash_animated_arrow( 'left', 'button', false ),
					'next_text' => sprintf( '<span class="screen-reader-text">%s</span>', esc_html__( 'Next page', 'bloghash' ) ) . bloghash_animated_arrow( 'right', 'button', false ),
				)
			);
		?>
		</div>
...
if ( ! function_exists( 'bloghash_link_pages' ) ) :
	/**
	 * Output the wp_link_pages.
	 *
	 * @since 1.0.0
	 */
	function bloghash_link_pages() {

		wp_link_pages(
			array(
				'before'      => '<div class="page-links"><em>' . esc_html__( 'Pages', 'bloghash' ) . '</em>',
				'after'       => '</div>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
			)
...
if ( ! function_exists( 'bloghash_excerpt' ) ) :
	/**
	 * Get excerpt.
	 *
	 * @since 1.0.0
	 ...
	 */
	function bloghash_excerpt( $length = null, $more = null ) {
		...
		if ( has_excerpt( $post->ID ) ) {
			...
			if ( strpos( $post->post_content, '<!--more-->' ) ) {
				...
				if ( null === $length ) {
					$length = apply_filters( 'excerpt_length', intval( bloghash_option( 'excerpt_length' ) ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
				}

				if ( null === $more ) {
					$more = apply_filters( 'excerpt_more', bloghash_option( 'excerpt_more' ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
				}
...
if ( ! function_exists( 'bloghash_entry_meta_author' ) ) :
	/**
	 ...
	 */
	function bloghash_entry_meta_author( $args = array() ) {

		$defaults = array(
			'show_avatar' => is_single() && bloghash_option( 'single_entry_meta_icons' ) || ! is_single() && bloghash_option( 'entry_meta_icons' ),
			'user_id'     => get_post_field( 'post_author', get_the_ID() ),
		);

		$args = wp_parse_args( $args, $defaults );
		$args = apply_filters( 'bloghash_entry_meta_author_args', $args );

		?>
		<span class="post-author">
			<span class="posted-by vcard author" <?php bloghash_schema_markup( 'author' ); ?>>
				<span class="screen-reader-text"><?php esc_html_e( 'Posted by', 'bloghash' ); ?></span>
				...
				<span>
					<a class="url fn n" title="<?php /* translators: %1$s Author */ printf( esc_attr__( 'View all posts by %1$s', 'bloghash' ), esc_attr( get_the_author() ) ); ?>" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID', $args['user_id'] ) ) ); ?>" rel="author" <?php bloghash_schema_markup( 'url' ); ?>>
						<span class="author-name" <?php bloghash_schema_markup( 'name' ); ?>><?php echo esc_html( get_the_author_meta( 'display_name', $args['user_id'] ) ); ?></span>
					</a>
				</span>
			</span>
		</span>
...
if ( ! function_exists( 'bloghash_entry_meta_date' ) ) :
	/**
	 ...
	 */
	function bloghash_entry_meta_date( $args = array() ) {

		$defaults = array(
			'show_published' => true,
			'show_modified'  => false,
			'modified_label' => esc_html__( 'Last updated on', 'bloghash' ),
			'date_format'    => '',
		...
		$args = wp_parse_args( $args, $defaults );
		$args = apply_filters( 'bloghash_entry_date_args', $args );

		// Icon.
		$icon = bloghash()->icons->get_meta_icon( 'date', bloghash()->icons->get_svg( 'calendar', array( 'aria-hidden' => 'true' ) ) );
...
if ( ! function_exists( 'bloghash_entry_meta_comments' ) ) :
	/**
	 * Prints HTML with meta information for the comments.
	 *
	 * @since 1.0.0
	 */
	function bloghash_entry_meta_comments() {

		$icon = bloghash()->icons->get_meta_icon( 'comments', bloghash()->icons->get_svg( 'message-square', array( 'aria-hidden' => 'true' ) ) );

		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';

			// wp_kses_post( $icon );
			comments_popup_link( wp_kses( $icon, bloghash_get_allowed_html_tags( 'post' ) ) . esc_html__( 'No Comments', 'bloghash' ), wp_kses( $icon, bloghash_get_allowed_html_tags( 'post' ) ) . esc_html__( '1', 'bloghash' ), wp_kses( $icon, bloghash_get_allowed_html_tags( 'post' ) ) . esc_html__( '%', 'bloghash' ), 'comments-link' );
...
if ( ! function_exists( 'bloghash_entry_meta_category' ) ) :
	/**
	 ...
	 */
	function bloghash_entry_meta_category( $sep = ', ', $show_icon = true, $limit_categories = -1, $return = false ) {
		...
		// Icon.
		$icon = $show_icon ? bloghash()->icons->get_meta_icon( 'category', bloghash()->icons->get_svg( 'folder-open', array( 'aria-hidden' => 'true' ) ) ) : '';
...
if ( ! function_exists( 'bloghash_entry_meta_tag' ) ) :
	/**
	 ...
	 * @param bool   $show_icon Show an icon for the meta detail.
	 */
	function bloghash_entry_meta_tag( $before = '<span class="cat-links"><span>', $sep = ', ', $after = '</span></span>', $id = 0, $show_icon = true ) {

		$icon = $show_icon ? bloghash()->icons->get_meta_icon( 'tags', bloghash()->icons->get_svg( 'tag', array( 'aria-hidden' => 'true' ) ) ) : '';

		// Add icon.
		$before = $before . wp_kses( $icon, bloghash_get_allowed_html_tags() );
...
if ( ! function_exists( 'bloghash_get_post_media' ) ) :
	 ...
	 */
	function bloghash_get_post_media( $post_format = false, $post = null ) {
		...
		switch ( $post_format ) {
			...
			case 'video':
				$return = bloghash_get_video_from_post( $post );
				break;

			case 'audio':
				$return = do_shortcode( bloghash_get_audio_from_post( $post ) );
				break;

			case 'gallery':
				$gallery = bloghash_get_post_gallery( $post );
			...
			case 'image':
			default:
				$size    = bloghash_option( 'blog_image_size' );
				$caption = false;
				...
				if ( is_single( $post ) || is_page( $post ) ) {
					...
					if ( 'no-sidebar' === bloghash_get_sidebar_position( $post ) ) {
						$size = 'full';
					}
				}

				if ( has_post_thumbnail( $post ) ) {
					$return = bloghash_get_post_thumbnail( $post, $size, $caption );
				} elseif ( 'image' === $post_format ) {
					$return = bloghash_get_image_from_post( $post );
				}

				break;
		}

		return apply_filters( 'bloghash_get_post_media', $return, $post_format, $post );
	}
endif;

if ( ! function_exists( 'bloghash_post_media' ) ) :
	 ...
	 * @param  string $post_format Post Format.
	 */
	function bloghash_post_media( $post_format = false ) {
		echo bloghash_get_post_media( $post_format ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
endif;

if ( ! function_exists( 'bloghash_top_bar_widget_text' ) ) :
	/**
	 ...
	 */
	function bloghash_top_bar_widget_text( $options ) {

		$content = isset( $options['content'] ) ? $options['content'] : '';
		$content = apply_filters( 'bloghash_dynamic_strings', $content );

		echo '<span>' . wp_kses( do_shortcode( $content ), bloghash_get_allowed_html_tags() ) . '</span>';
	}
endif;

if ( ! function_exists( 'bloghash_top_bar_widget_nav' ) ) :
	/**
	 ...
	 * @since 1.0.0
	 */
	function bloghash_top_bar_widget_nav( $options ) {

		$defaults = array(
			'menu_id'     => 'bloghash-topbar-nav',
			'container'   => false,
			'menu_class'  => false,
			'link_before' => '<span>',
			'link_after'  => '</span>',
		...
		$options = apply_filters( 'bloghash_top_bar_navigation_args', $options );

		if ( empty( $options['menu'] ) ) {
			if ( is_user_logged_in() && current_user_can( 'edit_theme_options' ) ) {
				?>
				<ul>
					<li class="bloghash-empty-nav">
						<?php
						if ( is_customize_preview() ) {
							esc_html_e( 'Menu not assigned', 'bloghash' );
						} else {
							?>
							<a href="<?php echo esc_url( admin_url( 'customize.php?autofocus[control]=bloghash_top_bar_widgets' ) ); ?>"><?php echo esc_html__( 'Assign a menu', 'bloghash' ); ?></a>
						<?php } ?>
					</li>
				</ul>
				<?php
		...
		$options['before_nav'] = '<nav class="bloghash-nav" role="navigation" aria-label="' . esc_attr( $options['menu'] ) . '">';
		$options['after_nav']  = '</nav>';

		bloghash_navigation( $options );
	}
endif;

if ( ! function_exists( 'bloghash_top_bar_widget_socials' ) ) :
	/**
	 ...
	 * @since 1.0.0
	 */
	function bloghash_top_bar_widget_socials( $options ) {
		bloghash_social_links( $options );
	}
endif;

if ( ! function_exists( 'bloghash_header_widget_text' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_header_widget_text( $options ) {
		bloghash_top_bar_widget_text( $options );
	}
endif;

if ( ! function_exists( 'bloghash_header_widget_advertisements' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_header_widget_advertisements( $options ) {
...
if ( ! function_exists( 'bloghash_header_widget_socials' ) ) :
	 ...
	 * @since 1.1.0
	 */
	function bloghash_header_widget_socials( $options ) {
		bloghash_social_links( $options );
	}
endif;

if ( ! function_exists( 'bloghash_header_widget_darkmode' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_header_widget_darkmode( $options ) {
		...
		echo wp_kses(
			sprintf(
				'<button type="button" class="bloghash-darkmode %1$s"><span></span></button>',
				esc_attr( $class )
			),
			bloghash_get_allowed_html_tags()
		);
...
if ( ! function_exists( 'bloghash_header_widget_search' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_header_widget_search( $options ) {
		get_template_part( 'template-parts/header/widgets/search' );
	}
endif;

if ( ! function_exists( 'bloghash_header_widget_button' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_header_widget_button( $options ) {
		...
		$class = apply_filters( 'bloghash_header_widget_button_class', $class );
		$class = trim( implode( ' ', $class ) );

		$text = empty( $options['text'] ) ? __( 'Add Button Text', 'bloghash' ) : $options['text'];
...
if ( ! function_exists( 'bloghash_about_widget_button' ) ) :
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_about_widget_button( $options ) {
		bloghash_header_widget_button( $options );
	}
endif;

if ( ! function_exists( 'bloghash_cta_widget_button' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_cta_widget_button( $options ) {
		bloghash_header_widget_button( $options );
	}
endif;

if ( ! function_exists( 'bloghash_cta_widget_text' ) ) :
	/**
	 ...
	 * @param array $options Array of widget options.
	 */
	function bloghash_cta_widget_text( $options ) {
		bloghash_top_bar_widget_text( $options );
	}
endif;

if ( ! function_exists( 'bloghash_copyright_widget_text' ) ) :
	/**
	 * Outputs the top bar text widget.
	 *
	 * @since 1.0.0
	 * @param array $options Array of widget options.
	 */
	function bloghash_copyright_widget_text( $options ) {
		bloghash_top_bar_widget_text( $options );
	}
endif;

if ( ! function_exists( 'bloghash_copyright_widget_nav' ) ) :
	/**
	 ...
	 * @since 1.0.0
	 */
	function bloghash_copyright_widget_nav( $options ) {

		$defaults = array(
			'menu_id'     => 'bloghash-footer-nav',
			'container'   => false,
			'menu_class'  => false,
			'link_before' => '<span>',
			'link_after'  => '</span>',
		...
		$options = wp_parse_args( $options, $defaults );
		$options = apply_filters( 'bloghash_copyright_navigation_args', $options );
				...
				<ul>
					<li class="bloghash-empty-nav">
						...
						if ( is_customize_preview() ) {
							...
							?>
							<a href="<?php echo esc_url( admin_url( 'customize.php?autofocus[control]=bloghash_copyright_widgets' ) ); ?>"><?php echo esc_html__( 'Assign a menu', 'bloghash' ); ?></a>
						<?php } ?>
					</li>
				</ul>
				<?php
		...
		$options['before_nav'] = '<nav role="navigation" class="bloghash-nav">';
		$options['after_nav']  = '</nav>';
...
if ( ! function_exists( 'bloghash_copyright_widget_socials' ) ) :
	/**
	 ...
	 * @since 1.0.0
	 */
	function bloghash_copyright_widget_socials( $options ) {
		bloghash_social_links( $options );
	}
endif;

if ( ! function_exists( 'bloghash_footer_widgets' ) ) :
	/**
	 * Outputs the footer widgets.
	 *
	 * @since 1.0.0
	 */
	function bloghash_footer_widgets() {

		$footer_layout  = bloghash_option( 'footer_layout' );
		$column_classes = bloghash_get_footer_column_class( $footer_layout );

		if ( is_array( $column_classes ) && ! empty( $column_classes ) ) {
			foreach ( $column_classes as $i => $column_class ) {

				$sidebar_id = 'bloghash-footer-' . ( $i + 1 );
				?>
				<div class="bloghash-footer-column <?php echo esc_attr( $column_class ); ?>">
					<?php
					if ( is_active_sidebar( $sidebar_id ) ) {
						...
						if ( current_user_can( 'edit_theme_options' ) ) {
							...
							$sidebar_name = bloghash_get_sidebar_name_by_id( $sidebar_id );
							?>
							<div class="bloghash-footer-widget bloghash-widget bloghash-no-widget">

								<div class='h4 widget-title'><?php echo esc_html( $sidebar_name ); ?></div>

								<p class='no-widget-text'>
									<?php if ( is_customize_preview() ) { ?>
										<a href='#' class="bloghash-set-widget" data-sidebar-id="<?php echo esc_attr( $sidebar_id ); ?>">
										<?php } else { ?>
...
if ( ! function_exists( 'bloghash_ad_widget_advertisement' ) ) :
	/**
	 * Outputs the header button widget.
	 *
	 * @since 1.0.0
...
if ( ! function_exists( 'bloghash_random_post_archive_advertisement_part' ) ) :
	function bloghash_random_post_archive_advertisement_part( $ads_rendered ) {
		...
		if ( ! empty( $ad_widgets ) && isset( $ad_widgets[ $ads_rendered ] ) ) :
			...
			$classes = apply_filters( 'bloghash_ad_widget_classes', $classes, $ad_widgets[ $ads_rendered ] );
			$classes = trim( implode( ' ', $classes ) );

			printf( '<div class="%s">', esc_attr( $classes ) );
			bloghash_ad_widget_advertisements( $ad_widgets[ $ads_rendered ]['values'] );
			printf( '</div>' );
...
if ( ! function_exists( 'bloghash_comment' ) ) :
	/**
	 ...
	 */
	function bloghash_comment( $comment, $args, $depth ) {
		...
		if ( 'pingback' === $comment->comment_type ) {
			...
			<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
				...
				<article id="comment-<?php comment_ID(); ?>" class="bloghash-pingback">
					<p><?php esc_html_e( 'Pingback: ', 'bloghash' ); ?><span<?php bloghash_schema_markup( 'author_name' ); ?>><?php comment_author_link(); ?></span> <?php edit_comment_link( esc_html__( '(Edit)', 'bloghash' ), '<span class="edit-link">', '</span>' ); ?></p>
				</article>
			...
			<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
				...
				<?php
							...
							bloghash_schema_markup( 'comment' );
				?>
							>
					...
					<header class="comment-header">
						<div class="comment-author vcard">
							...
							<span class="comment-author-avatar">
								...
								<?php if ( $comment->user_id === $post->post_author ) { ?>
									<span class="bypostauthor-badge" aria-hidden="true" title="<?php esc_attr_e( 'The post author', 'bloghash' ); ?>"><?php echo esc_html_x( 'A', 'Post author badge on comments', 'bloghash' ); ?></span>
								<?php } ?>
							</span>
						...
						<div class="comment-actions">
							<?php
							$bloghash_comment_reply_link = get_comment_reply_link(
								array_merge(
									$args,
									array(
										'depth'      => $depth,
							...
							?>
							<div class="edit">
								<?php edit_comment_link( __( 'Edit', 'bloghash' ) ); ?>
							</div>

							<?php
							if ( current_user_can( 'edit_comment', get_comment_ID() ) && null !== $bloghash_comment_reply_link ) {
								?>
								<span class="bloghash-comment-sep"></span>
								<?php
							}
							?>
							...
							<div class="reply">
								<?php
								echo $bloghash_comment_reply_link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
								?>
							</div>
						</div>
					...
					<div class="comment-content">
						<?php if ( '0' === $comment->comment_approved ) : ?>
							<p class="comment-awaiting-moderation"><em><?php esc_html_e( 'Your comment is awaiting moderation.', 'bloghash' ); ?></em></p>
						<?php endif; ?>
...
if ( ! function_exists( 'bloghash_social_links' ) ) :
	/**
	 ...
	 * @return void
	 */
	function bloghash_social_links( $args = array() ) {

		$defaults = array(
			'fallback_cb'     => '',
			'menu'            => '',
			'container'       => 'nav',
			'container_class' => 'bloghash-social-nav',
			'menu_class'      => 'bloghash-socials-menu',
			'depth'           => 1,
			'link_before'     => '<span class="screen-reader-text">',
			'link_after'      => '</span>' . bloghash()->icons->get_svg( 'external-link', array( 'aria-hidden' => 'true' ) ) . bloghash()->icons->get_svg(
				'external-link',
				array(
					'aria-hidden' => 'true',
					'class'       => 'bottom-icon',
		...
		$args = wp_parse_args( $args, $defaults );
		$args = apply_filters( 'bloghash_social_links_args', $args );
		...
		if ( ! empty( $args['size'] ) ) {
			$args['container_class'] .= ' bloghash-' . esc_attr( $args['size'] );
		}

		if ( ! empty( $args['menu'] ) && is_nav_menu( $args['menu'] ) ) {
			wp_nav_menu( $args );
		} else {
			echo '<nav class="bloghash-social-nav ' . esc_attr( $args['style'] ) . ' bloghash-' . esc_attr( $args['size'] ) . '">
			<ul id="menu-social-menu-default" class="bloghash-socials-menu">
				<li class="menu-item">
					<a href="https://www.facebook.com/">
						<span class="screen-reader-text">facebook.com</span>
						<span class="facebook">
						' . bloghash()->icons->get_svg( 'facebook', array( 'aria-hidden' => 'true' ) ) . '
				...
				<li class="menu-item">
					...
					<span class="screen-reader-text">twitter.com</span>
						<span class="twitter">
						' . bloghash()->icons->get_svg( 'twitter', array( 'aria-hidden' => 'true' ) ) . '
						' . bloghash()->icons->get_svg(
							'twitter',
							array(
								'aria-hidden' => 'true',
				...
				<li class="menu-item">
					<a href="https://t.me/">
						...
						<span class="telegram">
						' . bloghash()->icons->get_svg( 'telegram', array( 'aria-hidden' => 'true' ) ) . '
						' . bloghash()->icons->get_svg(
							'telegram',
							array(
								'aria-hidden' => 'true',
				...
				<li class="menu-item">
					<a href="https://www.instagram.com/">
						...
						' . bloghash()->icons->get_svg( 'instagram', array( 'aria-hidden' => 'true' ) ) . '
						' . bloghash()->icons->get_svg(
							'instagram',
							array(
								'aria-hidden' => 'true',
				...
				<li class="menu-item">
					<a href="https://youtube.com/">
						...
						<span class="youtube">
						' . bloghash()->icons->get_svg( 'youtube', array( 'aria-hidden' => 'true' ) ) . '
						' . bloghash()->icons->get_svg(
							'youtube',
							array(
								'aria-hidden' => 'true',
...
if ( ! function_exists( 'bloghash_navigation' ) ) :
	/**
	 * The template tag for displaying social icons.
	 *
	 * @param  array $args Args for wp_nav_menu function.
	 ...
	 * @return void
	 */
	function bloghash_navigation( $args = array() ) {
		...
		$args = apply_filters( 'bloghash_navigation_args', $args );
...
if ( ! function_exists( 'bloghash_breadcrumb' ) ) :
	/**
	 * Outputs breadcrumbs trail
	 *
	 * @param array $args Array of breadcrumb options.
	 */
	function bloghash_breadcrumb( $args = array() ) {
...
function bloghash_header_widget_output( $locations, $all_widgets ) {
	...
	if ( ! empty( $locations ) ) {
		...
		foreach ( $locations as $location ) {
			...
			$header_class = ' bloghash-widget-location-' . $location;
	...
	echo '<div class="bloghash-header-widgets bloghash-header-element' . esc_attr( $header_class ) . '">';

	if ( ! empty( $header_widgets ) ) {
		foreach ( $header_widgets as $location => $widgets ) {

			do_action( 'bloghash_header_widgets_before_' . $location );

			if ( ! empty( $widgets ) ) {
				foreach ( $widgets as $widget ) {
					if ( function_exists( 'bloghash_header_widget_' . $widget['type'] ) ) {

						$classes   = array();
						$classes[] = 'bloghash-header-widget__' . esc_attr( $widget['type'] );
						$classes[] = 'bloghash-header-widget';

						if ( isset( $widget['values']['visibility'] ) && $widget['values']['visibility'] ) {
							$classes[] = 'bloghash-' . esc_attr( $widget['values']['visibility'] );
						}

						$classes = apply_filters( 'bloghash_header_widget_classes', $classes, $widget );
						$classes = trim( implode( ' ', $classes ) );

						printf( '<div class="%s"><div class="bloghash-widget-wrapper">', esc_attr( $classes ) );
						call_user_func( 'bloghash_header_widget_' . $widget['type'], $widget['values'] );
						printf( '</div></div><!-- END .bloghash-header-widget -->' );
					}
				}
			}

			do_action( 'bloghash_header_widgets_after_' . $location );
		}
	}

	echo '</div><!-- END .bloghash-header-widgets -->';
}
